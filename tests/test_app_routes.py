...
import app as melodicmap_app


class AuthRoutingTests(unittest.TestCase):
    def setUp(self) -> None:
        melodicmap_app.app.config["TESTING"] = True
        melodicmap_app.app.config["SECRET_KEY"] = "test-secret"
        self.client = melodicmap_app.app.test_client()

    def test_callback_redirects_to_start_page_after_successful_auth(self) -> None:
        with self.client.session_transaction() as session:
            session[melodicmap_app.STATE_KEY] = "expected-state"

        fake_oauth = Mock()
        fake_oauth.get_access_token.return_value = {"access_token": "token"}

        with (
            patch.object(melodicmap_app, "_settings_from_env", return_value=object()),
            patch.object(melodicmap_app, "_build_oauth", return_value=fake_oauth),
        ):
        ...
        with self.client.session_transaction() as session:
            self.assertEqual(session[melodicmap_app.TOKEN_INFO_KEY]["access_token"], "token")
    ...
    def test_start_page_renders_auto_submit_form_for_authenticated_user(self) -> None:
        ...
        self.assertIn("Starting your MelodicMap process", body)
    ...
    def test_start_page_redirects_home_when_user_is_not_authenticated(self) -> None:
        with (
            patch.object(melodicmap_app, "_settings_from_env", return_value=object()),
            patch.object(melodicmap_app, "_get_authenticated_client", return_value=(None, None)),
        ):
            response = self.client.get("/start")
